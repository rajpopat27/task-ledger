...
func runParseTimeCases(t *testing.T, parserName string, now time.Time, tests []parseTimeCase, parse func(string, time.Time) (time.Time, error)) {
	t.Helper()
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := parse(tt.input, now)
			if (err != nil) != tt.wantErr {
				t.Errorf("%s(%q) error = %v, wantErr %v", parserName, tt.input, err, tt.wantErr)
				return
			}
			if tt.wantErr {
				return
			...
			if got.Year() != tt.wantYear {
				t.Errorf("%s(%q) year = %d, want %d", parserName, tt.input, got.Year(), tt.wantYear)
			}
			if got.Month() != tt.wantMonth {
				t.Errorf("%s(%q) month = %v, want %v", parserName, tt.input, got.Month(), tt.wantMonth)
			}
			if got.Day() != tt.wantDay {
				t.Errorf("%s(%q) day = %d, want %d", parserName, tt.input, got.Day(), tt.wantDay)
			}
			if tt.wantHour >= 0 && got.Hour() != tt.wantHour {
				t.Errorf("%s(%q) hour = %d, want %d", parserName, tt.input, got.Hour(), tt.wantHour)
			}
		})
	}
}
...
func TestParseNaturalLanguage(t *testing.T) {
	...
	runParseTimeCases(t, "ParseNaturalLanguage", now, tests, ParseNaturalLanguage)
}
...
func TestParseRelativeTime(t *testing.T) {
	...
	runParseTimeCases(t, "ParseRelativeTime", now, tests, ParseRelativeTime)
}
