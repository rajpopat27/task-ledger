...
func TestGetDependencyTree_IncludesRoot(t *testing.T) {
	...
	child := &types.Issue{
		...
		Dependencies: []*types.Dependency{
			{IssueID: "tl-7zka.2", DependsOnID: "tl-7zka", Type: "blocks"},
		},
	}
	...
	tree, err := m.GetDependencyTree(context.Background(), "tl-7zka.2", 50, false, false)
	if err != nil {
	...
	if tree[0].ID != "tl-7zka.2" {
		t.Errorf("Expected root ID 'tl-7zka.2', got '%s'", tree[0].ID)
	}
	if tree[0].Depth != 0 {
	...
	if tree[1].ID != "tl-7zka" {
		t.Errorf("Expected dependency ID 'tl-7zka', got '%s'", tree[1].ID)
	}
	if tree[1].Depth != 1 {
		t.Errorf("Expected dependency depth 1, got %d", tree[1].Depth)
	}
	if tree[1].ParentID != "tl-7zka.2" {
		t.Errorf("Expected dependency ParentID 'tl-7zka.2', got '%s'", tree[1].ParentID)
	}
}
