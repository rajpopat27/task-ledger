...
func setupTestIssueIndex(t *testing.T) *issueIndex {
	t.Helper()
...
func TestGetIssueNotFound(t *testing.T) {
	...
	ctx := context.Background()
	issue, err := store.GetIssue(ctx, "tl-999")
	if err != nil {
		t.Fatalf("GetIssue failed: %v", err)
	}
...
func TestCreateIssues(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			testStore := setupTestIssueIndex(t)
...
func TestLoadFromIssues(t *testing.T) {
	...
	issues := []*types.Issue{
		{
			...
			Dependencies: []*types.Dependency{{IssueID: "tl-1", DependsOnID: "tl-2", Type: types.DepBlocks}},
		},
		{
			ID:        "tl-2",
			Title:     "Issue 2",
	...
	ctx := context.Background()
	loaded, err := store.GetIssue(ctx, "tl-1")
	if err != nil {
		t.Fatalf("GetIssue failed: %v", err)
	}
	...
	if store.counters["tl"] != 2 {
		t.Errorf("Expected counter tl=2, got %d", store.counters["tl"])
	}
}
...
func TestIssueIndexLoadFromIssuesRejectsDuplicateExternalRef(t *testing.T) {
	store := newIssueIndex()
	defer store.Close()

	ref := "EXT-1"
	err := store.LoadFromIssues([]*types.Issue{
		{ID: "tk-a", Title: "A", Status: types.StatusOpen, Priority: 2, IssueType: types.TypeTask, ExternalRef: &ref},
		{ID: "tk-b", Title: "B", Status: types.StatusOpen, Priority: 2, IssueType: types.TypeTask, ExternalRef: &ref},
	})
	if err == nil {
		t.Fatal("LoadFromIssues succeeded with duplicate external_ref")
	}
	if !strings.Contains(err.Error(), "external_ref EXT-1") || !strings.Contains(err.Error(), "tk-a") || !strings.Contains(err.Error(), "tk-b") {
		t.Fatalf("error = %v, want external_ref and both IDs", err)
	}
	if got, _ := store.GetIssueByExternalRef(context.Background(), ref); got != nil {
		t.Fatalf("duplicate load mutated index: %#v", got)
	}
}

func TestIssueIndexCreateIssueRejectsDuplicateExternalRef(t *testing.T) {
	store := setupTestIssueIndex(t)
	defer store.Close()
	ctx := context.Background()
	...
	err := store.CreateIssue(ctx, &types.Issue{
		ID:          "tk-b",
		Title:       "B",
		Status:      types.StatusOpen,
		Priority:    2,
	...
	if err == nil {
		t.Fatal("CreateIssue succeeded with duplicate external_ref")
	}
	if !strings.Contains(err.Error(), "external_ref EXT-1 already belongs to tk-a") {
		t.Fatalf("error = %v", err)
	}
}

func TestIssueIndexCreateIssuesRejectsDuplicateExternalRefInBatch(t *testing.T) {
	store := setupTestIssueIndex(t)
	defer store.Close()
	ctx := context.Background()

	ref := "EXT-1"
	err := store.CreateIssues(ctx, []*types.Issue{
		{ID: "tk-a", Title: "A", Status: types.StatusOpen, Priority: 2, IssueType: types.TypeTask, ExternalRef: &ref},
		{ID: "tk-b", Title: "B", Status: types.StatusOpen, Priority: 2, IssueType: types.TypeTask, ExternalRef: &ref},
	}, "test-user")
	if err == nil {
		t.Fatal("CreateIssues succeeded with duplicate external_ref")
	}
	if got, _ := store.GetIssue(ctx, "tk-a"); got != nil {
		t.Fatalf("batch create mutated index: %#v", got)
	}
}

func TestIssueIndexUpdateIssueRejectsDuplicateExternalRef(t *testing.T) {
	store := setupTestIssueIndex(t)
	defer store.Close()
	ctx := context.Background()
	...
	if err := store.UpdateIssue(ctx, "tk-b", map[string]interface{}{"external_ref": ref}, "test-user"); err == nil {
		t.Fatal("UpdateIssue succeeded with duplicate external_ref")
	}
	got, err := store.GetIssueByExternalRef(ctx, ref)
	if err != nil {
		t.Fatalf("GetIssueByExternalRef: %v", err)
	}
	if got == nil || got.ID != "tk-a" {
		t.Fatalf("external ref moved after failed update: %#v", got)
	}
}
...
func TestGetIssueByExternalRefLoadFromIssues(t *testing.T) {
	...
	if found1 == nil || found1.ID != "tl-1" {
		t.Errorf("Expected to find tl-1 by external ref jira#100")
	}
	...
	if found2 == nil || found2.ID != "tl-2" {
		t.Errorf("Expected to find tl-2 by external ref jira#200")
	}
}
...
func TestGetNextChildID_ConfigurableMaxDepth(t *testing.T) {
	...
	issues := []struct {
		...
		{"tl-depth", "Root"},
		{"tl-depth.1", "Level 1"},
		{"tl-depth.1.1", "Level 2"},
		{"tl-depth.1.1.1", "Level 3"},
	}
	...
	_, err = store.GetNextChildID(ctx, "tl-depth.1.1")
	if err == nil {
		t.Errorf("expected error for depth 3 with max-depth=2, got nil")
	}
	if err != nil && err.Error() != "maximum hierarchy depth (2) exceeded for parent tl-depth.1.1" {
		t.Errorf("unexpected error message: %v", err)
	}
}
