...
// TempDirOnFastFS creates a temporary directory that preferentially uses
// ramdisk-backed filesystems when available. This reduces I/O
...
func TempDirOnFastFS(t testing.TB) string {
	t.Helper()
	...
	case "linux":
		...
		if stat, err := os.Stat("/dev/shm"); err == nil && stat.IsDir() {
			...
			tmpBase := filepath.Join("/dev/shm", "taskledger-test")
	...
	tmpDir, err := os.MkdirTemp(baseDir, "taskledger-test-*")
	if err != nil {
