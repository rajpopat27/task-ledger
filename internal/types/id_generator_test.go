...
func TestGenerateHashID_Deterministic(t *testing.T) {
	...
	hash1 := GenerateHashID("tl", "Title", "Desc", now, "ws1")
	hash2 := GenerateHashID("tl", "Title", "Desc", now, "ws1")
...
func TestGenerateHashID_DifferentInputs(t *testing.T) {
	...
	baseHash := GenerateHashID("tl", "Title", "Desc", now, "ws1")
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			hash := GenerateHashID("tl", tt.title, tt.description, tt.created, tt.workspaceID)
			if hash == baseHash {
...
func TestGenerateChildID(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			parentID:    "tl-af78e9a2",
			childNumber: 1,
			want:        "tl-af78e9a2.1",
		},
		{
			name:        "second level child",
			parentID:    "tl-af78e9a2.1",
			childNumber: 2,
			want:        "tl-af78e9a2.1.2",
		},
		{
			name:        "third level child",
			parentID:    "tl-af78e9a2.1.2",
			childNumber: 3,
			want:        "tl-af78e9a2.1.2.3",
		},
		{
			name:        "large child number",
			parentID:    "tl-af78e9a2",
			childNumber: 347,
			want:        "tl-af78e9a2.347",
		},
	}
...
func TestParseHierarchicalID(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			id:         "tl-af78e9a2.1.2",
			wantRoot:   "tl-af78e9a2",
			wantParent: "tl-af78e9a2.1",
			wantDepth:  2,
		},
		{
			name:       "third level child",
			id:         "tl-af78e9a2.1.2.3",
			wantRoot:   "tl-af78e9a2",
			wantParent: "tl-af78e9a2.1.2",
			wantDepth:  3,
		},
	}
...
func BenchmarkGenerateHashID(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = GenerateHashID("tl", "Fix auth bug", "Users can't log in", now, "workspace-1")
	}
}
...
func BenchmarkGenerateChildID(b *testing.B) {
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		GenerateChildID("tl-af78e9a2", 42)
	}
}
...
func TestCheckHierarchyDepth(t *testing.T) {
	tests := []struct {
		...
		{"root parent with default depth", "tl-abc123", 0, false, ""},
		{"depth 1 parent with default depth", "tl-abc123.1", 0, false, ""},
		{"depth 2 parent with default depth", "tl-abc123.1.2", 0, false, ""},
		{"depth 3 parent with default depth - exceeds", "tl-abc123.1.2.3", 0, true, "maximum hierarchy depth (3) exceeded for parent tl-abc123.1.2.3"},

		// Custom maxDepth
		{"root parent with max=1", "tl-abc123", 1, false, ""},
		{"depth 1 parent with max=1 - exceeds", "tl-abc123.1", 1, true, "maximum hierarchy depth (1) exceeded for parent tl-abc123.1"},
		{"depth 3 parent with max=5", "tl-abc123.1.2.3", 5, false, ""},
		{"depth 4 parent with max=5", "tl-abc123.1.2.3.4", 5, false, ""},
		{"depth 5 parent with max=5 - exceeds", "tl-abc123.1.2.3.4.5", 5, true, "maximum hierarchy depth (5) exceeded for parent tl-abc123.1.2.3.4.5"},

		// Negative maxDepth falls back to default
		{"negative maxDepth uses default", "tl-abc123.1.2.3", -1, true, "maximum hierarchy depth (3) exceeded for parent tl-abc123.1.2.3"},
	}
