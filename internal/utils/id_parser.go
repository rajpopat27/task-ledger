...
// If the input already has the prefix (e.g., "tl-a3f8e9"), returns it as-is.
// If the input lacks the prefix (e.g., "a3f8e9"), adds the configured prefix.
// Works with hierarchical IDs too: "a3f8e9.1.2" → "tl-a3f8e9.1.2"
...
// Supports:
// - Full IDs: "tl-a3f8e9" or "a3f8e9" → "tl-a3f8e9"
// - Without hyphen: "bda3f8e9" or "wya3f8e9" → "tl-a3f8e9"
// - Partial IDs: "a3f8" → "tl-a3f8e9" (if unique match)
// - Hierarchical: "a3f8e9.1" → "tl-a3f8e9.1"
//
// Returns an error if:
...
func ResolvePartialID(ctx context.Context, store storage.Storage, input string) (string, error) {
	...
	// This uses the same query path as "tl list --id", ensuring consistency.
	...
	// Normalize input:
	// 1. If it has the full prefix with hyphen (tl-a3f8e9), use as-is
	...
	if strings.HasPrefix(input, prefixWithHyphen) {
		// Already has prefix with hyphen: "tl-a3f8e9"
		normalizedID = input
	} else {
