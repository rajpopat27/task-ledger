...
//   - "tl-multi-part-id" -> "tl" (word-like suffix: "id" is too short but "part-id" path)
//
...
func ExtractIssuePrefix(issueID string) string {
	// Try last hyphen first (handles multi-part prefixes like "taskledger-vscode-1")
	...
	if len(suffix) == 0 {
		// Trailing hyphen like "tl-" - return prefix before the hyphen
...
//
// Hash IDs in taskledger use adaptive length scaling from 3-8 characters.
...
// ExtractIssueNumber extracts the number from an issue ID like "tl-123" -> 123
