...
//
// Regression coverage for upstream issue 446.
...
// TestExtractIssuePrefixWordSuffix tests word-like suffixes (tl-fasa regression)
...
// This fixes tl-fasa regression where word-like suffixes were wrongly treated as hashes.
func TestExtractIssuePrefixWordSuffix(t *testing.T) {
	...
	wordSuffixes := []struct {
		...
		{"vc-baseline-test", "vc"},  // tl-fasa: "baseline-test" is the ID, not "test"
		{"vc-baseline-hello", "vc"}, // tl-fasa: "baseline-hello" is the ID
		{"vc-some-feature", "vc"},   // tl-fasa: "some-feature" is the ID
	}
