...
// particularly for TASK_LEDGER_DIR environment variable processing and git worktree
...
func resolveCanonicalCase(path string) string {
	if runtime.GOOS == "darwin" {
		...
		cmd := exec.Command("realpath", path) // #nosec G204 -- executable is fixed; path is passed as an argument, not shell-evaluated.
...
// paths in project discovery and git helpers.
