...
func TestShouldUsePager(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "TL_NO_PAGER env set",
			opts:      PagerOptions{NoPager: false},
			envVars:   map[string]string{"TL_NO_PAGER": "1"},
			wantPager: false,
		},
	}
...
func TestGetPagerCommand(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "TL_PAGER set",
			envVars:   map[string]string{"TL_PAGER": "more"},
			wantPager: "more",
		},
		{
			name:      "PAGER set",
		...
		{
			name:      "TL_PAGER takes precedence over PAGER",
			envVars:   map[string]string{"TL_PAGER": "more", "PAGER": "cat"},
			wantPager: "more",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			oldBdPager := os.Getenv("TL_PAGER")
			oldPager := os.Getenv("PAGER")
			os.Unsetenv("TL_PAGER")
			os.Unsetenv("PAGER")
			defer func() {
				if oldBdPager != "" {
					os.Setenv("TL_PAGER", oldBdPager)
				}
				if oldPager != "" {
...
func TestToPagerWithBdNoPagerEnv(t *testing.T) {
	oldVal := os.Getenv("TL_NO_PAGER")
	os.Setenv("TL_NO_PAGER", "1")
	defer func() {
		if oldVal != "" {
			os.Setenv("TL_NO_PAGER", oldVal)
		} else {
			os.Unsetenv("TL_NO_PAGER")
		}
	}()
