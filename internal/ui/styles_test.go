...
func TestRenderBasicStyles(t *testing.T) {
	t.Run("semantic wrappers", func(t *testing.T) {
		cases := []struct {
			...
			{"command", RenderCommand("tl prime"), CommandStyle.Render("tl prime")},
		}
...
func TestRenderIssueCompact(t *testing.T) {
	open := RenderIssueCompact("tl-1", 0, "bug", "in_progress", "ship it")
	...
	closed := RenderIssueCompact("tl-2", 2, "task", "closed", "done")
	raw := fmt.Sprintf("%s [P%d] [%s] %s - %s", "tl-2", 2, "task", "closed", "done")
...
func TestRenderClosedUtilities(t *testing.T) {
	...
	if got := RenderID("tl-5"); got != IDStyle.Render("tl-5") {
...
func TestRenderCommandAndCategoryAreUppercaseSafe(t *testing.T) {
	...
	cmd := RenderCommand("tl prime")
	if !strings.Contains(cmd, "tl prime") {
...
func TestIsAgentMode(t *testing.T) {
	...
	t.Setenv("TL_AGENT_MODE", "")
	t.Setenv("CLAUDE_CODE", "")
	if IsAgentMode() {
		t.Fatal("expected false with no env vars")
	}

	// Test TL_AGENT_MODE=1
	t.Setenv("TL_AGENT_MODE", "1")
	t.Setenv("CLAUDE_CODE", "")
	if !IsAgentMode() {
		t.Fatal("expected true with TL_AGENT_MODE=1")
	}
