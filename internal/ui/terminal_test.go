...
func TestShouldUseEmoji(t *testing.T) {
	...
	origNoEmoji := os.Getenv("TL_NO_EMOJI")
	defer setEnv("TL_NO_EMOJI", origNoEmoji)
	...
	tests := []struct {
		...
		{
			name:      "TL_NO_EMOJI disables emoji",
			noEmoji:   "1",
			wantEmoji: false,
		},
		{
			name:      "No TL_NO_EMOJI falls back to TTY check",
			noEmoji:   "",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			os.Unsetenv("TL_NO_EMOJI")
			if tt.noEmoji != "" {
				os.Setenv("TL_NO_EMOJI", tt.noEmoji)
			}
