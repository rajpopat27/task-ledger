// Package ui provides terminal styling and pager support for taskledger CLI output.
package ui
...
// - TL_NO_PAGER environment variable is set
...
// Checks TL_PAGER, then PAGER, defaults to "less".
func getPagerCommand() string {
	if pager := os.Getenv("TL_PAGER"); pager != "" {
		return pager
	}
