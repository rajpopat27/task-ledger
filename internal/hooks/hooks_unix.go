...
func (r *Runner) runHook(hookPath, event string, issue *types.Issue) error {
	...
	// #nosec G204 -- hookPath is from controlled .task-ledger/hooks directory
	...
	// callers blocked. Creating a process group and sending a negative PID to
	// syscall.Kill ensures the entire group is killed reliably on timeout.
