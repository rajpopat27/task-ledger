...
AGENT_NAME="${TASK_LEDGER_AGENT_NAME:-bash-agent-$$}"

# Check if tl is installed
if ! command -v tl &> /dev/null; then
    log_error "tl is not installed"
    echo "Install or build tl and make sure it is available on PATH"
    exit 1
fi

# Check if we're in a tl-initialized directory
if ! tl list &> /dev/null; then
    log_error "Not in a tl-initialized directory"
    echo "Run: tl init"
    exit 1
fi
...
# Find ready work
find_ready_work() {
    tl ready --json --limit 1 2>/dev/null | jq -r '.[0] // empty'
}
...
claim_task() {
    ...
    tl update "$issue_id" --status in_progress --json > /dev/null
...
do_work() {
    ...
    local parent_id=$(get_field "$issue" "parent_id")
    ...
    if [[ "$title" != Follow-up:* && $((RANDOM % 2)) -eq 0 ]]; then
        log_warning "Discovered issue while working!"

        # Create new issue
        local create_args=(create task "Follow-up: $title"
...
complete_task() {
    ...
    tl close "$issue_id" --reason "$reason" --json > /dev/null
...
run_agent() {
    ...
    echo ""
    echo "🚀 Task Ledger Agent starting..."
