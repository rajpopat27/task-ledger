#!/usr/bin/env python3
"""
Simple AI agent workflow using tl (Task Ledger issue tracker).
...
class TaskLedgerAgent:
    """Simple agent that manages tasks using tl."""

    def __init__(self):
        self.current_task = None

    def run_tl(self, *args) -> dict:
        """Run tl command and parse JSON output."""
        cmd = ["tl"] + list(args) + ["--json"]
    ...
    def find_ready_work(self) -> Optional[dict]:
        ...
        ready = self.run_tl("ready", "--limit", "1")
    ...
    def claim_task(self, issue_id: str) -> dict:
        ...
        result = self.run_tl("update", issue_id, "--status", "in_progress")
        return result
    ...
    def create_issue(self, title: str, description: str = "",
        ...
        args = ["create", issue_type, title, "-p", str(priority)]
        if self.current_task and self.current_task.get("parent_id"):
            args.extend(["--parent", self.current_task["parent_id"]])
        if description:
    ...
    def link_discovery(self, discovered_id: str, parent_id: str):
        ...
        subprocess.run(
            ["tl", "dep", "add", discovered_id, parent_id, "--type", "discovered-from"],
            check=True
        )
    ...
    def complete_task(self, issue_id: str, reason: str = "Completed"):
        ...
        result = self.run_tl("close", issue_id, "--reason", reason)
        return result
    ...
    def simulate_work(self, issue: dict) -> bool:
        ...
        if ("implement" in normalized_title or "add" in normalized_title) and not normalized_title.startswith("add tests for"):
    ...
    def run(self, max_iterations: int = 10):
        ...
        print("🚀 Task Ledger Agent starting...\n")
...
def main():
    ...
    try:
        ...
        print(f"Error running tl: {e}", file=sys.stderr)
        print("Make sure tl is installed and available on PATH")
        sys.exit(1)
