...
Examples:
  tl count                          # Count all issues
  tl count --status open            # Count open issues
  tl count --by-status              # Group count by status
  tl count --by-priority            # Group count by priority
  tl count --by-type                # Group count by issue type
  tl count --by-assignee            # Group count by assignee
  tl count --by-label               # Group count by label
  tl count --assignee alice --by-status  # Count alice's issues by status
`,
